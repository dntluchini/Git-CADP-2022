...
procedure MayorRelacion (e : esc; var cantEscuelas : integer);
begin
	 if (relacion (e.cantAlumnos,e.cantDocentes) > 23.435) and (e.localidad = 'lp') then
	   cantEscuelas += 1;
end;

procedure cueYNomMayor (max1,max2: integer; e: esc ; var maxcue1,maxcue2 : integer; var maxnom1,maxnom2 : string);
begin
  if (relacion (e.cantAlumnos,e.cantDocentes) > max1) then begin
   maxcue2 := maxcue1;
   maxcue1 := e.cue;
   maxnom2 := maxnom1;
   maxnom1 := e.nombre;
   ...
   if (relacion (e.cantAlumnos,e.cantDocentes) > max2) then begin
    maxcue2 := e.cue;
    maxnom2 := e.nombre;
   end;
 end;
...
var
 e : esc;
 i, max1,max2,cuemax1,cuemax2,escuelasCant : integer;
 nommax1,nommax2 : string;
begin
	max1 := -9999; max2 := -9999; escuelasCant := 0;
	for i := 1 to 3 do begin
	 leer (e);
	 MayorRelacion (e,escuelasCant);
	 cueYNomMayor (max1,max2,e,cuemax1,cuemax2,nommax1,nommax2);
	end;
